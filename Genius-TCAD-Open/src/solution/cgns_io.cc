...
void CGNSIO::write (const std::string& filename)
{
  ...
  for( unsigned int r=0; r<system.n_regions(); r++)
  {
    ...
    if( Genius::processor_id() == 0)
    {
      ...
      {
        int cg_err = cg_poly_section_write(fn, B ,Z, "GridElements", MIXED, 1, elem_id.size(), 0, &elem_package[0], &elem_offsets[0], &S);
        if(cg_err) { std::cerr << "[" << Genius::processor_id() << "] cg_poly_section_write failed for zone '" << zone_name << "' (" << elem_id.size() << " elems, " << elem_package.size() << " connectivity entries): " << cg_get_error() << std::endl; genius_assert(!cg_err); }
      }
#ifdef ENABLE_AMR
          ...
          if( Genius::processor_id() == 0)
          {
            ...
            for( region_data_it = region_data.begin(); region_data_it != region_data.end(); )
            {
              ...
              while (bounds.first != bounds.second)
              {
                ...
                {
                  std::vector<double> fdata = _sort_it(bounds.first->second.second, region_node_local_id);
                  genius_assert(!fdata.empty());
                  int cg_err = cg_field_write(fn, B, Z, SOL, RealDouble, variable.c_str(), &fdata[0], &F);
                  if(cg_err) { std::cerr << "[" << Genius::processor_id() << "] cg_field_write failed for field '" << variable << "': " << cg_get_error() << std::endl; genius_assert(!cg_err); }
                }
                genius_assert(!cg_gopath(fn, path.c_str()));
