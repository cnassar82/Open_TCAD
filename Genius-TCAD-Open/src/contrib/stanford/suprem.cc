...
bool SupremTIF::read(std::string &err)
{
  ...
  while(ctmp >> flag)
  {
    ...
    else if (flag == "n")
    {
      ...
      // SUPREM writes solution node ids zero-based; c/t/r records are one-based.
  ...
  // Map SUPREM doping species to the keys that acceptor()/donor() look up.
  // Prefer the electrically-active concentrations; fall back to total if absent.
  if(_sol_head.has_solution("BoronActive"))
    _solution_index_map.insert(std::make_pair("B_index", _sol_head.solution_index("BoronActive")));
  else if(_sol_head.has_solution("Boron"))
    _solution_index_map.insert(std::make_pair("B_index", _sol_head.solution_index("Boron")));

  if(_sol_head.has_solution("PhosphorusActive"))
    _solution_index_map.insert(std::make_pair("P_index", _sol_head.solution_index("PhosphorusActive")));
  else if(_sol_head.has_solution("Phosphorus"))
    _solution_index_map.insert(std::make_pair("P_index", _sol_head.solution_index("Phosphorus")));

  if(_sol_head.has_solution("ArsenicActive"))
    _solution_index_map.insert(std::make_pair("As_index", _sol_head.solution_index("ArsenicActive")));
  else if(_sol_head.has_solution("Arsenic"))
    _solution_index_map.insert(std::make_pair("As_index", _sol_head.solution_index("Arsenic")));

  if(_sol_head.has_solution("AntimonyActive"))
    _solution_index_map.insert(std::make_pair("Sb_index", _sol_head.solution_index("AntimonyActive")));
  else if(_sol_head.has_solution("Antimony"))
    _solution_index_map.insert(std::make_pair("Sb_index", _sol_head.solution_index("Antimony")));

  if(_sol_head.has_solution("AluminumActive"))
    _solution_index_map.insert(std::make_pair("Al_index", _sol_head.solution_index("AluminumActive")));
  else if(_sol_head.has_solution("Aluminum"))
    _solution_index_map.insert(std::make_pair("Al_index", _sol_head.solution_index("Aluminum")));
